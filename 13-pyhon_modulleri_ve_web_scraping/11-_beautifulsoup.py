...
# Sayfanın içeriği hakkında bilgi almamızı sağlar.

soup = BeautifulSoup(html_doc, 'html.parser')

result = soup.prettify()
#Konsol üzerinden düzeltilmiş halde görmemizi sağlar

result = soup.title
result = soup.head
result = soup.body

result = soup.title.name         # etiketi gösterir
result = soup.title.string       # title içerisindeki ifadeyi gösterir

result = soup.h1
result = soup.h2          # birden fazla var ise ilkini getirir.
result = soup.h2.name
result = soup.h2.string
result = soup.h1.string

result = soup.find_all('h2')     # Bütün etiketleri getirir
...
# divin alt elemanlarının hepsini getirir

result = soup.div.findNextSibling().findNextSibling().findPreviousSibling()
# findNextSibling => bir sonraki dive geçmesini sağlar
# findPreviousSibling => bir öncekine gitmesini sağlar
